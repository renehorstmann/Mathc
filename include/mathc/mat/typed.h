...
static vec3f mat33f_get_row(mat33f mat, int row) {
    vec3f res;
    matf_get_row(res.v, mat.m[0], row, 3);
    return res;
}
...
static mat33f mat33f_transpose(mat33f mat) {
    mat33f res;
    matf_transpose_no_alias(res.m[0], mat.m[0], 3);
    return res;
}
...
static mat33f mat33f_mul_mat(mat33f mat_a, mat33f mat_b) {
    mat33f res;
    matf_mul_mat_no_alias(res.m[0], mat_a.m[0], mat_b.m[0], 3);
    return res;
}
...
static vec3f mat33f_mul_vec(mat33f mat_a, vec3f vec_b) {
    vec3f res;
    matf_mul_vec_no_alias(res.v, mat_a.m[0], vec_b.v, 3);
    return res;
}
...
static vec4f mat44f_get_row(mat44f mat, int row) {
    vec4f res;
    matf_get_row(res.v, mat.m[0], row, 4);
    return res;
}
...
static mat44f mat44f_transpose(mat44f mat) {
    mat44f res;
    matf_transpose_no_alias(res.m[0], mat.m[0], 4);
    return res;
}
...
static mat44f mat44f_mul_mat(mat44f mat_a, mat44f mat_b) {
    mat44f res;
    matf_mul_mat_no_alias(res.m[0], mat_a.m[0], mat_b.m[0], 4);
    return res;
}
...
static vec4f mat44f_mul_vec(mat44f mat_a, vec4f vec_b) {
    vec4f res;
    matf_mul_vec_no_alias(res.v, mat_a.m[0], vec_b.v, 4);
    return res;
}
...
static vec3d mat33d_get_row(mat33d mat, int row) {
    vec3d res;
    matd_get_row(res.v, mat.m[0], row, 3);
    return res;
}
...
static mat33d mat33d_transpose(mat33d mat) {
    mat33d res;
    matd_transpose_no_alias(res.m[0], mat.m[0], 3);
    return res;
}
...
static mat33d mat33d_mul_mat(mat33d mat_a, mat33d mat_b) {
    mat33d res;
    matd_mul_mat_no_alias(res.m[0], mat_a.m[0], mat_b.m[0], 3);
    return res;
}
...
static vec3d mat33d_mul_vec(mat33d mat_a, vec3d vec_b) {
    vec3d res;
    matd_mul_vec_no_alias(res.v, mat_a.m[0], vec_b.v, 3);
    return res;
}
...
static vec3d vec3d_mul_mat(vec3d vec_a, mat33d mat_b) {
    vec3d res;
    vecd_mul_mat_no_alias(res.v, vec_a.v, mat_b.m[0], 3);
    return res;
}

/** returns = det mat<3*3> mat33 */
static double mat33d_determinant(mat33d mat) {
    return matd_determinant33(mat.m[0]);
}

/** mat<3*3> dst = inv(mat<3*3> mat33) */
static mat33d mat33d_invert(mat33d mat) {
    mat33d res;
    matd_invert33_no_alias(res.m[0], mat.m[0]);
    return res;
}
...
static vec4d mat44d_get_row(mat44d mat, int row) {
    vec4d res;
    matd_get_row(res.v, mat.m[0], row, 4);
    return res;
}
...
static mat44d mat44d_transpose(mat44d mat) {
    mat44d res;
    matd_transpose_no_alias(res.m[0], mat.m[0], 4);
    return res;
}
...
static mat44d mat44d_mul_mat(mat44d mat_a, mat44d mat_b) {
    mat44d res;
    matd_mul_mat_no_alias(res.m[0], mat_a.m[0], mat_b.m[0], 4);
    return res;
}
...
static vec4d mat44d_mul_vec(mat44d mat_a, vec4d vec_b) {
    vec4d res;
    matd_mul_vec_no_alias(res.v, mat_a.m[0], vec_b.v, 4);
    return res;
}
...
static vec4d vec4d_mul_mat(vec4d vec_a, mat44d mat_b) {
    vec4d res;
    vecd_mul_mat_no_alias(res.v, vec_a.v, mat_b.m[0], 4);
    return res;
}

/** returns = det mat<4*4> mat44 */
static double mat44d_determinant(mat44d mat) {
    return matd_determinant44(mat.m[0]);
}

/** mat<4*4> dst = inv(mat<4*4> mat44) */
static mat44d mat44d_invert(mat44d mat) {
    mat44d res;
    matd_invert44_no_alias(res.m[0], mat.m[0]);
    return res;
}
...
static vec3i mat33i_get_row(mat33i mat, int row) {
    vec3i res;
    mati_get_row(res.v, mat.m[0], row, 3);
    return res;
}
...
static mat33i mat33i_transpose(mat33i mat) {
    mat33i res;
    mati_transpose_no_alias(res.m[0], mat.m[0], 3);
    return res;
}
...
static mat33i mat33i_mul_mat(mat33i mat_a, mat33i mat_b) {
    mat33i res;
    mati_mul_mat_no_alias(res.m[0], mat_a.m[0], mat_b.m[0], 3);
    return res;
}
...
static vec3i mat33i_mul_vec(mat33i mat_a, vec3i vec_b) {
    vec3i res;
    mati_mul_vec_no_alias(res.v, mat_a.m[0], vec_b.v, 3);
    return res;
}
...
static vec3i vec3i_mul_mat(vec3i vec_a, mat33i mat_b) {
    vec3i res;
    veci_mul_mat_no_alias(res.v, vec_a.v, mat_b.m[0], 3);
    return res;
}

/** returns = det mat<3*3> mat33 */
static int mat33i_determinant(mat33i mat) {
    return mati_determinant33(mat.m[0]);
}

/** mat<3*3> dst = inv(mat<3*3> mat33) */
static mat33i mat33i_invert(mat33i mat) {
    mat33i res;
    mati_invert33_no_alias(res.m[0], mat.m[0]);
    return res;
}
...
static vec4i mat44i_get_row(mat44i mat, int row) {
    vec4i res;
    mati_get_row(res.v, mat.m[0], row, 4);
    return res;
}
...
static mat44i mat44i_transpose(mat44i mat) {
    mat44i res;
    mati_transpose_no_alias(res.m[0], mat.m[0], 4);
    return res;
}
...
static mat44i mat44i_mul_mat(mat44i mat_a, mat44i mat_b) {
    mat44i res;
    mati_mul_mat_no_alias(res.m[0], mat_a.m[0], mat_b.m[0], 4);
    return res;
}
...
static vec4i mat44i_mul_vec(mat44i mat_a, vec4i vec_b) {
    vec4i res;
    mati_mul_vec_no_alias(res.v, mat_a.m[0], vec_b.v, 4);
    return res;
}
...
static vec4i vec4i_mul_mat(vec4i vec_a, mat44i mat_b) {
    vec4i res;
    veci_mul_mat_no_alias(res.v, vec_a.v, mat_b.m[0], 4);
    return res;
}

/** returns = det mat<4*4> mat44 */
static int mat44i_determinant(mat44i mat) {
    return mati_determinant44(mat.m[0]);
}

/** mat<4*4> dst = inv(mat<4*4> mat44) */
static mat44i mat44i_invert(mat44i mat) {
    mat44i res;
    mati_invert44_no_alias(res.m[0], mat.m[0]);
    return res;
}
