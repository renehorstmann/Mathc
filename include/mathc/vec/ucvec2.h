...
/** returns the index of the min vec value */
static int ucvec2_min_index_v(const unsigned char *vec) {
    return ucvec2_min_index(UCVec2(vec));
}
...
/** returns the index of the max vec value */
static int ucvec2_max_index_v(const unsigned char *vec) {
    return ucvec2_max_index(UCVec2(vec));
}
...
/** dst = x < min ? min : (x > max ? max : x) */
static ucvec2
ucvec2_clamp_vec_v(const unsigned char *vec_x, const unsigned char *vec_min, const unsigned char *vec_max) {
...
/** returns dot(b-a) */
static float ucvec2_sqr_distance(ucvec2 vec_a, ucvec2 vec_b) {
    return ucvecN_sqr_distance(vec_a.v, vec_b.v, 2);
}

/** returns dot(b-a) */
static float ucvec2_sqr_distance_v(const unsigned char *vec_a, const unsigned char *vec_b) {
    return ucvec2_sqr_distance(UCVec2(vec_a), UCVec2(vec_b));
}
