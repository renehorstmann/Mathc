...
/** dst = a + b * c */
static void ivecN_add_scaled_vec(int *dst, const int *a, const int *b, const int *c, int n) {
    for (int i = 0; i < n; i++)
        dst[i] = a[i] + b[i] * c[i];
}

/** dst = a + b * c */
static void ivecN_add_scaled(int *dst, const int *a, const int *b, int c, int n) {
    for (int i = 0; i < n; i++)
        dst[i] = a[i] + b[i] * c;
}
