...
static int uvec3_min_index(uvec3 vec) {
    return uvecN_min_index(vec.v, 3);
}

/** returns the index of the min vec value */
static int uvec3_min_index_v(const unsigned *vec) {
    return uvec3_min_index(UVec3(vec));
}

/** returns the index of the max vec value */
static int uvec3_max_index(uvec3 vec) {
    return uvecN_max_index(vec.v, 3);
}

/** returns the index of the max vec value */
static int uvec3_max_index_v(const unsigned *vec) {
    return uvec3_max_index(UVec3(vec));
}
...
/** returns dot(b-a) */
static float uvec3_sqr_distance(uvec3 vec_a, uvec3 vec_b) {
    return uvecN_sqr_distance(vec_a.v, vec_b.v, 3);
}

/** returns dot(b-a) */
static float uvec3_sqr_distance_v(const unsigned *vec_a, const unsigned *vec_b) {
    return uvec3_sqr_distance(UVec3(vec_a), UVec3(vec_b));
}
