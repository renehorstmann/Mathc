...
/** returns the index of the min vec value */
static int vec4_min_index_v(const float *vec) {
    return vec4_min_index(Vec4(vec));
}
...
/** returns the index of the max vec value */
static int vec4_max_index_v(const float *vec) {
    return vec4_max_index(Vec4(vec));
}
...
/** returns dot(b-a) */
static float vec4_sqr_distance(vec4 vec_a, vec4 vec_b) {
    return vecN_sqr_distance(vec_a.v, vec_b.v, 4);
}

/** returns dot(b-a) */
static float vec4_sqr_distance_v(const float *vec_a, const float *vec_b) {
    return vec4_sqr_distance(Vec4(vec_a), Vec4(vec_b));
}
