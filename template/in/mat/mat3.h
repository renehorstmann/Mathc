...
/** m[:][col] = v; dst = m */
static mat3 mat3_set_this_col(mat3 *m, vec3 v, int col) {
    matN_set_col(m->v, v.v, col, 3);
    return *m;
}
...
/** m[row][:] = s; dst = m */
static mat3 mat3_set_this_row_sca_v(float *m, float s, int row) {
    return mat3_set_this_row_sca((mat3 *) m, s, row);
}
...
/** m[:][col] = s; dst = m */
static mat3 mat3_set_this_col_sca_v(float *m, float s, int col) {
    return mat3_set_this_col_sca((mat3 *) m, s, col);
}
