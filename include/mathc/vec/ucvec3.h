...
/** returns the index of the min vec value */
static int ucvec3_min_index_v(const unsigned char *vec) {
    return ucvec3_min_index(UCVec3(vec));
}
...
/** returns the index of the max vec value */
static int ucvec3_max_index_v(const unsigned char *vec) {
    return ucvec3_max_index(UCVec3(vec));
}
...
/** dst = x < min ? min : (x > max ? max : x) */
static ucvec3
ucvec3_clamp_vec_v(const unsigned char *vec_x, const unsigned char *vec_min, const unsigned char *vec_max) {
...
/** returns dot(b-a) */
static float ucvec3_sqr_distance(ucvec3 vec_a, ucvec3 vec_b) {
    return ucvecN_sqr_distance(vec_a.v, vec_b.v, 3);
}

/** returns dot(b-a) */
static float ucvec3_sqr_distance_v(const unsigned char *vec_a, const unsigned char *vec_b) {
    return ucvec3_sqr_distance(UCVec3(vec_a), UCVec3(vec_b));
}
