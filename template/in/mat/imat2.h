...
/** m[row][:] = v; dst = m */
static imat2 imat2_set_this_row(imat2 *m, v ivec, int row) {
    imatN_set_row(m->v, v.v, row, 2);
    return *m;
}

/** dst = m; dst[:][col] = v */
static imat2 imat2_set_col(imat2 m, ivec2 ivec, int col) {
    imatN_set_col(m.v, ivec.v, col, 2);
    return m;
}

/** m[:][col] = v; dst = m */
static imat2 imat2_set_this_col(imat2 *m, ivec2 v, int col) {
...
/** m[row][:] = s; dst = m */
static imat2 imat2_set_this_row_sca_v(int *m, int s, int row) {
    return imat2_set_this_row_sca((imat2 *) m, s, row);
}
...
/** dst = a @ b */
static imat2 imat2_mul_imat(imat2 a, imat2 b) {
    imat2 res;
    imatN_mul_imat_no_alias(res.v, a.v, b.v, 2);
    return res;
}


/** dst = a @ b */
static ivec2 imat2_mul_vec(imat2 a, ivec2 ivec_b) {
    ivec2 res;
    imatN_mul_vec_no_alias(res.v, a.v, ivec_b.v, 2);
    return res;
}


/** dst = a @ b */
static ivec2 ivec2_mul_imat(ivec2 ivec_a, imat2 b) {
    ivec2 res;
    ivecN_mul_imat_no_alias(res.v, ivec_a.v, b.v, 2);
    return res;
}
