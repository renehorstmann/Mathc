...
/** m[row][:] = v; dst = m */
static mat2 mat2_set_this_row(mat2 *m, v vec, int row) {
    matN_set_row(m->v, v.v, row, 2);
    return *m;
}

/** dst = m; dst[:][col] = v */
static mat2 mat2_set_col(mat2 m, vec2 vec, int col) {
    matN_set_col(m.v, vec.v, col, 2);
    return m;
}

/** m[:][col] = v; dst = m */
static mat2 mat2_set_this_col(mat2 *m, vec2 v, int col) {
...
/** m[row][:] = s; dst = m */
static mat2 mat2_set_this_row_sca_v(float *m, float s, int row) {
    return mat2_set_this_row_sca((mat2 *) m, s, row);
}
...
/** dst = a @ b */
static vec2 mat2_mul_vec(mat2 a, vec2 vec_b) {
    vec2 res;
    matN_mul_vec_no_alias(res.v, a.v, vec_b.v, 2);
    return res;
}


/** dst = a @ b */
static vec2 vec2_mul_mat(vec2 vec_a, mat2 b) {
    vec2 res;
    vecN_mul_mat_no_alias(res.v, vec_a.v, b.v, 2);
    return res;
}
